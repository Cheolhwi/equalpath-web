...
- Nine browser scenarios passed across contact priority, results, price/registration and comparison. Contact-priority and comparison scenarios passed again after removing the internal-rule explanation, including assertions that the explanation is absent. Desktop search/comparison and 390 px mobile-map screenshots in `.build/contact-priority/` were visually inspected; two contactable candidates produce exactly two highlighted cards/pins, while the no-contact second page uses the fallback. These browser data are controlled fixtures.
- Final public query deployment `6aa73602d590994092ec` is ready. Live distance and price requests for KL Sentral on 2026-09-14, 13:00–18:00, each returned the same 20 of 394 candidates within 10 km. Each page contained 10 contactable candidates without known conflicts; all three suggestions had contact details, and the internal contact rule was absent from ordering copy. Live comparison was also checked on the preceding deployment with the same ranking code. Aggregate evidence: `.build/contact-priority/live-api.json` and `evidence/api-deployment.json`. No provider data or contact actions were changed.
- Frontend release source digest: `03c08c154591ab5c8cb10d967319c9d8ac60b68fddd374a0c58a9aa5cd5d72ef`. Public site publication is verified separately after the push.
