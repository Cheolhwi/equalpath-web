...
test('restored map points gain a street address and priority highlights switch, tie, and survive removing a centre',async({page})=>{
  ...
  await expect(page.locator('.comparison-priority-message')).toContainText('share the same value');
  await page.getByRole('combobox',{name:'Comparison priority',exact:true}).click();await page.getByRole('option',{name:'Pickup service first',exact:true}).click();await expect(highlighted.locator('.comparison-best-tag')).toHaveText(['Pickup listed','Pickup listed']);
  ...
  await page.locator('.compare-pair > summary').click(); await page.getByRole('combobox',{name:'Centre 2',exact:true}).click(); await page.getByRole('option',{name:'Test tied',exact:true}).click();
