...
async function setup(page,items){
  ...
  await page.goto('/?care=short_term#discover');await page.locator('#service-date').fill('2026-09-21');await page.locator('#deadline').fill('13:00');await page.locator('#care-end').fill('17:00');await page.locator('#transport').selectOption('self');
  await page.getByRole('button',{name:'Find care options',exact:true}).click();await expect(page.locator('.provider-row').first()).toBeVisible();return calls;
}
...
for(const width of [1440,390])test(`${width}px: changing to an area without quotes clears price order and explains the disabled choice`,async({page})=>{
  ...
  await page.getByRole('button',{name:'Edit request',exact:true}).click();await page.getByRole('button',{name:'Update results',exact:true}).click();
