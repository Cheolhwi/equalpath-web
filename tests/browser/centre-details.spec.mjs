...
test('details put useful facts and next action first, retain evidence and prioritise mismatches',async({page})=>{
  ...
  await expect(dialog.locator('.care-journey')).toContainText('18:00');await expect(dialog.locator('.centre-metrics')).toContainText('MYR 35 / hour');
  ...
  await expect(dialog.getByRole('button',{name:'Contact the centre',exact:true})).toHaveCount(1);
  const within=await dialog.getByRole('button',{name:'Contact the centre',exact:true}).evaluate(el=>{const r=el.getBoundingClientRect();return r.top>0&&r.bottom<innerHeight;});expect(within).toBe(true);
  ...
  await dialog.getByRole('button',{name:'Contact the centre',exact:true}).click();await expect(page.getByRole('heading',{name:'Contact the centre',exact:true})).toBeVisible();
});
