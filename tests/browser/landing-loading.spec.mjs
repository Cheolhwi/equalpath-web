...
test("cold artwork stays behind the animated loader until the first drawn frame; the opening then runs visibly", async ({ page }) => {
  ...
  await expect(page.getByRole("status")).toHaveText("Loading artwork…");
  ...
  expect(await page.locator(".landing-loader-cards > span").first().evaluate(el => getComputedStyle(el).animationName)).toBe("loading-card");
...
test("failed artwork has a retry, and reduced-motion loading stays still", async ({ page }) => {
  ...
  await page.goto("/", { waitUntil: "domcontentloaded" });
  expect(await page.locator(".landing-loader-cards > span").first().evaluate(el => getComputedStyle(el).animationName)).toBe("none");
  await expect(page.getByRole("status")).toHaveText("The artwork couldn’t load.", { timeout: 60000 });
