...
export function nearbyCacheKey(body) {
  return JSON.stringify([body.mode ?? "live", body.careType ?? "short_term", searchRadius(body.radius, body.careType ?? "short_term"), Number(body.center?.lat).toFixed(3), Number(body.center?.lng).toFixed(3)]);
}
