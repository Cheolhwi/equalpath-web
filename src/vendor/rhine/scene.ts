...
export class ArchiveScene {
  ...
  constructor(
    ...
    this.renderer.domElement.setAttribute(
      "aria-label",
      "Interactive care collection. Drag to explore; select a card to lift it.",
    );
  ...
  private drawLabel(index: number) {
    ...
    c.fillText("EQUALPATH CARE", 22, 116);
    c.font = "32px MiSans";
    c.fillStyle = "#878476";
    c.fillText("PLAY · READ · GROW", 25, 174);
    c.fillStyle = "#171713";
    ...
    c.fillStyle = "#eee9de";
    c.font = "24px MiSans";
    c.fillText("E P / C A", 809, 61);
    c.fillStyle = "#171713";
    c.font = "bold 64px MiSans";
    c.fillText("CARE", 830, 143);
  ...
  private navigatePlane(coordinate: DragPosition) {
    ...
    try {
      ...
      for (let i = 1; i <= steps; i++) {
        ...
        if (!sameCell(cell, this.selectedCell)) this.onSelect?.(fileAtCell(cell), cell, "browse");
      }
    } finally {
    ...
    canvas.addEventListener("pointerup", (e) => {
      ...
      if (!cancelled && browse && this.canBrowse()) {
        ...
        if (this.archiveDrag.active) {
          ...
          const cell = this.pickCell(e.clientX, e.clientY);
          if (cell) this.onSelect?.(fileAtCell(cell), cell, "activate");
        }
      }
      reset();
    }, { signal: this.inputEvents.signal });
