...
export default function Enquiry({ p, request, selection, onSelection, onCompare, onPreparation }) {
  ...
  return <div className="enquiry-page">
    ...
    <section className="request-context enquiry-visit" aria-label="Centre and visit">
      ...
      <h3>{p.name}</h3>
      <dl><div><dt>Your date</dt><dd>{visitDate(request.date)}</dd></div><div><dt>Collect by</dt><dd>{request.deadline}</dd></div><div><dt>Care until</dt><dd>{request.end}</dd></div></dl>
    ...
    <div className="enquiry-layout">
      <section className="enquiry-questions" aria-labelledby={`${uid}-questions`}>
        ...
        <p className="enquiry-guide">{linkedCount} {linkedCount === 1 ? "question linked" : "questions linked"} to your checks · {routineCount} for every visit</p>
      ...
      <aside className="enquiry-tools" aria-label="Copy and contact">
        <section className="enquiry-send" aria-label="Your question list">
          ...
          <p className="enquiry-copy-status" role="status">{copyState === "copied" ? "Copied with your date, times and pickup place." : !selected.length ? "Select at least one question to copy." : "Your date, times and pickup place are included."}</p>
