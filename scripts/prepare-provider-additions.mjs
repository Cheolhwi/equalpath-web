...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,arg('evidence-dir','webapp-data/source/temporary-care-expansion-20260915'));
...
const review=JSON.parse(readFileSync(resolve(root,arg('review','webapp/data/short-care-additions-20260915.json'))));
const captures=['crawl.json','crawl-extra.json','crawl-maps.json','crawl-final.json'].filter(f=>existsSync(resolve(out,f))).flatMap(f=>JSON.parse(readFileSync(resolve(out,f))));
...
const source=key=>{
 ...
 const {pageURL,...metadata}=s;
 return {...metadata,url:pageURL??c.url,retrievedAt:c.retrievedAt,sourceDate:s.sourceDate??null,snapshotHash:c.sha256,...pageURL?{captureURL:c.url,captureMethod:'published_page_asset'}:{}};
};
const previous=arg('previous',null);
const retained=previous?JSON.parse(readFileSync(resolve(root,previous))).records:[];
const additions=review.providers.map(r=>{
 ...
 if(r.requirements?.length)admission.requirements=r.requirements;
...
const records=[...retained,...additions].sort((a,b)=>a.provider_id.localeCompare(b.provider_id));
const digest=additionsHash(records);
applyProviderAdditions(raw,records,manifest.release_id,digest);
const summary={baseRelease:manifest.release_id,release:'additions_'+digest.slice(0,24),hash:digest,count:records.length,retained:retained.length,added:additions.length,published:records.length,withPhone:records.filter(r=>r.provider.public_phone).length,withFees:records.filter(r=>r.provider.fees.length).length,withHours:records.filter(r=>r.provider.care_windows.length||r.provider.operating_hours.weekly_windows.length).length,preparedAt:new Date().toISOString(),unmergedLeads:review.unmergedLeads};
if(previous)summary.expectedPriorRelease=JSON.parse(readFileSync(resolve(root,previous))).summary.release;
...
writeFileSync(resolve(root,arg('output','webapp/data/prepared-short-care-additions-20260915.json')),JSON.stringify({summary,records},null,2));
console.log(JSON.stringify({...summary,unmergedLeads:summary.unmergedLeads.length}));

// A discovered centre already in the register gets new service evidence, not a duplicate branch.
if(review.existingProviders?.length){
 const prior=JSON.parse(readFileSync(resolve(root,arg('previous-admissions','webapp-data/source/temporary-care-20260915/prepared-admissions.json'))));
 const updates=review.existingProviders.map(r=>{
  const p=raw.find(p=>p.id===r.id);
  if(!p || p.official_name!==r.name || prior.records.some(x=>x.provider_id===r.id))throw Error('Existing service update must be a newly reviewed catalogue match');
  return {provider_id:r.id,base_release:manifest.release_id,match:{status:'matched',catalogue_name:r.name,basis:r.identityReview},status:'published',scope:'branch',service_types:r.serviceTypes,wording:r.wording,question:r.question,requirements:r.requirements,sources:r.admissionSources.map(source)};
 });
 const rows=[...prior.records,...updates].sort((a,b)=>a.provider_id.localeCompare(b.provider_id));
 const digest=additionsHash(rows);
 applyAdmissionsEvidence(raw,rows,manifest.release_id,digest);
 const summary={baseRelease:manifest.release_id,release:'admissions_'+digest.slice(0,24),hash:digest,count:rows.length,expectedPriorRelease:prior.summary.release,published:rows.filter(r=>r.status==='published').length,added:updates.length,retained:prior.records.length,preparedAt:new Date().toISOString()};
 const prepared={summary,records:rows};
 writeFileSync(resolve(out,'prepared-admissions.json'),JSON.stringify(prepared,null,2));
 writeFileSync(resolve(root,arg('admissions-output','webapp/data/prepared-short-care-admissions-deeper-20260915.json')),JSON.stringify(prepared,null,2));
