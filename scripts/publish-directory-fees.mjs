// Append a versioned, public fee evidence snapshot; never update owner tables.
import {readFileSync,writeFileSync} from 'node:fs';
import {resolve} from 'node:path';
import {createHash} from 'node:crypto';
import {spawnSync} from 'node:child_process';
import {applyFeesEvidence} from '../server/fees-overlay.mjs';
const root=resolve(import.meta.dirname,'../..'),backend=resolve(root,'appwrite-backend'),out=resolve(root,'webapp-data/source/directory-fees-20260913');
const prepared=JSON.parse(readFileSync(resolve(out,'prepared-fees.json'))),{summary,records}=prepared;
const raw=JSON.parse(readFileSync(resolve(root,'webapp-data/prepared/catalog.json'))).map(r=>JSON.parse(r.payload));
applyFeesEvidence(raw,records,summary.baseRelease,summary.hash);
const rows=records.map(r=>({$id:'wf_'+createHash('sha256').update(summary.release+':'+r.provider_id).digest('hex').slice(0,30),provider_id:r.provider_id,release_id:summary.release,payload:JSON.stringify(r)}));
...
const payload=JSON.stringify({...JSON.parse(before.payload),fees_release:summary.release,fees_hash:summary.hash,fees_count:rows.length,fees_published_at:new Date().toISOString()});
