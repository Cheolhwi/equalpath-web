// Append a versioned, public provider additions snapshot; never update owner tables.
import {readFileSync,writeFileSync} from 'node:fs';
import {resolve} from 'node:path';
import {createHash} from 'node:crypto';
import {spawnSync} from 'node:child_process';
import {applyProviderAdditions} from '../server/provider-additions.mjs';
const evidenceDir=process.argv.find(a=>a.startsWith('--evidence-dir='))?.slice(15)??'webapp-data/source/temporary-care-expansion-20260915';
const root=resolve(import.meta.dirname,'../..'),backend=resolve(root,'appwrite-backend'),out=resolve(root,evidenceDir);
const prepared=JSON.parse(readFileSync(resolve(out,'prepared-additions.json'))),{summary,records}=prepared;
const raw=JSON.parse(readFileSync(resolve(out,'raw-before.json')));
applyProviderAdditions(raw,records,summary.baseRelease,summary.hash);
if(summary.release!=='additions_'+summary.hash.slice(0,24)||summary.count!==records.length||!records.length)throw Error('Invalid prepared snapshot');
const rows=records.map(r=>({$id:'wp_'+createHash('sha256').update(summary.release+':'+r.provider_id).digest('hex').slice(0,30),provider_id:r.provider_id,release_id:summary.release,payload:JSON.stringify(r)}));
...
if(summary.expectedPriorRelease && JSON.parse(before.payload).additions_release!==summary.expectedPriorRelease)throw Error('Additions changed; prepare again');
writeFileSync(resolve(out,'publication-manifest-before.json'),JSON.stringify(before,null,2));
...
const payload=JSON.stringify({...JSON.parse(before.payload),additions_release:summary.release,additions_hash:summary.hash,additions_count:rows.length,additions_published_at:new Date().toISOString()});
