// Append a versioned, public service evidence snapshot; never update owner tables.
import {readFileSync,writeFileSync} from 'node:fs';
import {resolve} from 'node:path';
import {createHash} from 'node:crypto';
import {spawnSync} from 'node:child_process';
import {applyServicesEvidence} from '../server/services-overlay.mjs';
const root=resolve(import.meta.dirname,'../..'),backend=resolve(root,'appwrite-backend'),out=resolve(root,'webapp-data/source/services-20260913');
const prepared=JSON.parse(readFileSync(resolve(out,'prepared-services.json'))),{summary,records}=prepared;
const raw=JSON.parse(readFileSync(resolve(root,'webapp-data/prepared/catalog.json'))).map(r=>JSON.parse(r.payload));
applyServicesEvidence(raw,records,summary.baseRelease,summary.hash);
const rows=records.map(r=>({$id:'ws_'+createHash('sha256').update(summary.release+':'+r.provider_id).digest('hex').slice(0,30),provider_id:r.provider_id,release_id:summary.release,payload:JSON.stringify(r)}));
...
const payload=JSON.stringify({...JSON.parse(before.payload),services_release:summary.release,services_hash:summary.hash,services_count:rows.length,services_published_at:new Date().toISOString()});
