// Append a versioned, public temporary-care evidence snapshot; never update owner tables.
import {readFileSync,writeFileSync} from 'node:fs';
import {resolve} from 'node:path';
import {createHash} from 'node:crypto';
import {spawnSync} from 'node:child_process';
import {applyAdmissionsEvidence} from '../server/admissions-overlay.mjs';
const evidenceDir=process.argv.find(a=>a.startsWith('--evidence-dir='))?.slice(15)??'webapp-data/source/temporary-care-20260915';
const root=resolve(import.meta.dirname,'../..'),backend=resolve(root,'appwrite-backend'),out=resolve(root,evidenceDir);
const prepared=JSON.parse(readFileSync(resolve(out,'prepared-admissions.json'))),{summary,records}=prepared;
const raw=JSON.parse(readFileSync(resolve(out,'raw-before.json')));
applyAdmissionsEvidence(raw,records,summary.baseRelease,summary.hash);
if(summary.release!=='admissions_'+summary.hash.slice(0,24)||summary.count!==records.length||!records.length)throw Error('Invalid prepared snapshot');
const rows=records.map(r=>({$id:'wa_'+createHash('sha256').update(summary.release+':'+r.provider_id).digest('hex').slice(0,30),provider_id:r.provider_id,release_id:summary.release,payload:JSON.stringify(r)}));
...
if(summary.expectedPriorRelease && JSON.parse(before.payload).admissions_release!==summary.expectedPriorRelease)throw Error('Admissions changed; prepare again');
...
const payload=JSON.stringify({...JSON.parse(before.payload),admissions_release:summary.release,admissions_hash:summary.hash,admissions_count:rows.length,admissions_published_at:new Date().toISOString()});
