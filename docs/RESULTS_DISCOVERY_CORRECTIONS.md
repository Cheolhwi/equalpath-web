...
- At most three mapped, conflict-free centres per result page are marked **Suggested first** on both map and list. The selected distance/care-end/pickup strategy ranks first; relevant supported checks and distance break ties. Alphabetical display retains condition-based suggestions. Unselected age and unrequested institutional pickup do not inflate the score. These are suggestions to check, not accepted bookings. Other pins remain selectable.
- Comparison highlights the best conflict-free column(s) for the selected strategy: minimum straight-line distance, latest known care end for the requested date, or published pickup. Equal values share the highlight. Missing facts and known conflicts never receive a best-choice label. Alphabetical sorting has no best-choice claim. Switching priority or removing a centre recalculates the highlights.
