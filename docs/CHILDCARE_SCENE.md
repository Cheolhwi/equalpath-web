...
The 2026-09-13 revision follows the user's explicit request to use the original RhineLabUI 3D scene with childcare elements. The earlier map backdrop and standalone illustration proposal are superseded.

## Real upstream scene

- Repository: https://github.com/LBEILC/RhineLabUI
- Pinned commit: `d9ecb6c6f7a36e8b522072a0ebbd7691a50550a7`.
- Runtime: upstream `ArchiveScene`, glass materials, depth of field, lighting, array layout, long-lens camera, card extraction, drag and selection behavior. The host adapts framing to a dedicated landing viewport.
...
- Added physical props: a picture book and three rounded A/B/C blocks on the selected cassette. Original shell, fasteners, array and optics remain. The printed label uses EqualPath; the scene's sample items are decorative care themes, not provider data.

## Image asset

- Project asset: [childcare-book-cover.png](../public/images/childcare-book-cover.png).
- Generated using the built-in ImageGen tool; the image is used as a small picture-book texture inside the real 3D scene, not as the landing scene itself. It depicts fictional illustrated people.
...
- Canvas buttons provide previous/next objects and collection/close-up views. Collection drag changes the selected cell without treating a drag as a click; a click opens the close-up.
- No original audio, boot sequence, archived research documents, authentication or database mutations were included.

## Verification

Actual desktop and mobile checks are recorded in [design-qa.md](../design-qa.md) and `evidence/childcare-scene/`. Release validation checks the lazy scene chunk, both binary model headers and the picture-book texture as well as the existing app checks.
