...
- Entry uses a 1200 ms right-to-left ivory curtain: 480 ms to cover the landing, then 720 ms to reveal the same mounted search interface with a small horizontal settle. A clip reveals the stationary EQUALPATH wordmark and the currently selected artwork in a fine paper frame, arranged side by side on desktop and vertically on phones. Freeze the cover at entry and pause the scene’s automatic progression. This adapts the sidebar-navigation direction from the user’s [ignoredone.space reference](https://www.ignoredone.space/index.php/graphic-design-arknights/) while keeping the existing childcare artwork and visual palette. Reduced motion enters immediately and Escape completes entry. It does not wait for the model or a backend response.
- Transition images mount and decode while the landing is visible. Their request mode matches the scene textures, and entry reuses the same decoded elements rather than making a new image request. If a cover cannot load/decode, skip the decorative transition and enter immediately; never show an empty frame or block access.
