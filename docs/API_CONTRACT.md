...
The price-sort revision keeps the existing range-first pipeline: geographic inclusion → condition checks and explicit filters → nearest-distance page selection (at most 20) → conflict grouping and selected priority within the page. It does not choose 20 records from the whole region and filter afterwards. In-scope conflicting records remain eligible for their nearest page, rank after non-conflicts on that page, and appear as muted, selectable map pins. JKM and KPM use an identical check-seal icon; their provenance is distinguished by the disclosure text, not different institution icons.

Search responses include `ordering.pageSelection: "nearest"`; the ordering explanation states that the selected strategy applies within the current nearest-20 page. Suggestions and driving calls only use those selected page members. Total counts cover all eligible centres within the selected radius.
