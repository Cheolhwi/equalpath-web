...
- The user additionally requested a RhineLab-inspired landing page with Gaussian blur on 2026-09-12. The user subsequently simplified the entrance to one brief movement directly into the main interface: no zoom-out/pan sequence, intermediate screen or transfer labels. Preserve the same mounted flat map, reduced-motion controls, and in-memory request/comparison state when returning home. No automatic geolocation on the landing page.
