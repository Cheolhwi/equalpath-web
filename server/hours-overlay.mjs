...
export function applyHoursEvidence(raw, records, baseRelease, expectedHash) {
 ...
 for(const r of records){
  if(!ids.has(r.provider_id)||seen.has(r.provider_id)||r.base_release!==baseRelease||r.match?.status!=='matched'||!safeURL(r.source_url)||!Number.isFinite(Date.parse(r.observed_on))||!Array.isArray(r.weekly_windows)||!r.weekly_windows.length||!Array.isArray(r.closed_weekdays)||!['waze_public_place','google_maps_public_search','provider_website'].includes(r.source_kind))throw Error('Invalid hours evidence');
 ...
 return raw.map(p=>{
  ...
  if(existing.length||parsePublishedHours(h.notes).windows.length)return p;
